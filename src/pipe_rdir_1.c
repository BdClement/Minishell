...
/*   Updated: 2023/08/28 18:01:30 by clbernar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pipe_first(t_pipex vars, int i, t_command *tmp)
{
	...
	if (last_outfile != NULL && ft_strncmp(last_outfile->str, "/dev/stdout", len + 1))
	{
		if (dup2(last_outfile->fd, STDOUT_FILENO) < 0)
			exit(EXIT_FAILURE);
	}
	else if ((!last_outfile || last_outfile) && vars.nb_pipes > 0)
	{
...
void	pipe_last(t_pipex vars, int i, t_command *tmp)
{
	...
	init_last_rdirs(&last_infile, &last_outfile, &(tmp->redirections));
	if (last_infile != NULL)
		len = ft_strlen(last_infile->str);
	if (last_infile != NULL && ft_strncmp(last_infile->str, "/dev/stdin", len + 1))
	{
		if (dup2(last_infile->fd, STDIN_FILENO) < 0)
			exit(EXIT_FAILURE);
	}
	else if ((!last_infile || last_infile) && vars.nb_pipes > 0)
	{
